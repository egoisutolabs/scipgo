...
func (r Row) NNonZeroes() int { return int(C.SCIProwGetNNonz(r.raw)) }
...
func (r Row) Index() int { return int(C.SCIProwGetIndex(r.raw)) }

// Lhs returns the left-hand side of the row.
func (r Row) Lhs() float64 { return float64(C.SCIProwGetLhs(r.raw)) }

// Rhs returns the right-hand side of the row.
func (r Row) Rhs() float64 { return float64(C.SCIProwGetRhs(r.raw)) }

// Dual returns the dual value of the row.
func (r Row) Dual() float64 { return float64(C.SCIProwGetDualsol(r.raw)) }

// FarkasDual returns the Farkas dual value of the row.
func (r Row) FarkasDual() float64 { return float64(C.SCIProwGetDualfarkas(r.raw)) }

// BasisStatus returns the basis status of the row.
func (r Row) BasisStatus() BasisStatus { return basisStatusFromC(C.SCIProwGetBasisStatus(r.raw)) }

// Name returns the name of the row.
func (r Row) Name() string { return goString(C.SCIProwGetName(r.raw)) }

// Age returns the age of the row.
func (r Row) Age() int { return int(C.SCIProwGetAge(r.raw)) }

// Rank returns the rank of the row.
func (r Row) Rank() int { return int(C.SCIProwGetRank(r.raw)) }

// IsLocal returns whether the row is local.
func (r Row) IsLocal() bool { return C.SCIProwIsLocal(r.raw) != 0 }

// IsModifiable returns whether the row is modifiable.
func (r Row) IsModifiable() bool { return C.SCIProwIsModifiable(r.raw) != 0 }

// IsRemovable returns whether the row is removable.
func (r Row) IsRemovable() bool { return C.SCIProwIsRemovable(r.raw) != 0 }

// IsIntegral returns whether the row is integral; the activity of an integral
// row (without the constant) is always integral.
func (r Row) IsIntegral() bool { return C.SCIProwIsIntegral(r.raw) != 0 }

// OriginType returns the origin type of the row.
func (r Row) OriginType() RowOrigin { return rowOriginFromC(C.SCIProwGetOrigintype(r.raw)) }
...
func (r Row) IsInGlobalCutPool() bool { return C.SCIProwIsInGlobalCutpool(r.raw) != 0 }

// IsInLP returns whether the row is in the current LP.
func (r Row) IsInLP() bool { return C.SCIProwIsInLP(r.raw) != 0 }
...
// introduced.
func (r Row) Depth() int { return int(C.SCIProwGetLPDepth(r.raw)) }

// ActiveLPCount returns the number of times that this row has been sharp in
// an optimal LP solution.
func (r Row) ActiveLPCount() int { return int(C.SCIProwGetActiveLPCount(r.raw)) }

// NLpSinceCreate returns the number of LPs since this row has been created.
func (r Row) NLpSinceCreate() int { return int(C.SCIProwGetNLPsAfterCreation(r.raw)) }

// SetRank sets the rank of the row.
func (r *Row) SetRank(rank int) { C.SCIProwChgRank(r.raw, C.int(rank)) }

// SetCoeff sets the coefficient of a variable in the row.
func (r *Row) SetCoeff(v Variable, coeff float64) {
	mustOK(C.SCIPaddVarToRow(r.scip.raw, r.raw, v.raw, C.double(coeff)))
}
